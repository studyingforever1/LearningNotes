...
### Q19：fork() 到底做了什么？修改父进程内容会发生什么？

![fork() 完整流程：内核步骤 + COW 内存演化](./svg/fork-complete-flow.svg)

#### do_fork() 内核步骤

`fork()` 的系统调用入口最终调用 `copy_process()`，分五步完成：
...
copy_process()
       ...
       ├── ② 复制 mm_struct（内存描述符）
       │       复制所有 VMA（vm_area_struct 链表/红黑树）
       │       复制页表项（PTE） ← 只复制表项，不分配物理页！
       │
       ├── ③ 将父子所有可写页的 PTE 标记为只读（COW 标记）
       │       代码段本身只读，保持不变，直接共享
       │       数据段、堆、栈 → PTE 改为只读，等待写操作触发 COW
       │
       ├── ④ 所有共享物理页的引用计数 +1
       │       page->_refcount++，记录有多少 PTE 指向该页
       │
       └── ⑤ 复制其他资源
               文件描述符表（浅拷贝，fd 共享同一 struct file）
               信号处理表、命名空间、cgroup 等

返回：父进程得到子进程 PID，子进程得到 0
```

**fork 的真实开销**：复制页表是 O(VMA 数量)，而非 O(内存大小)。10GB 堆的进程 fork 只需数毫秒，物理内存增量约为 0。
...
物理内存
┌─────────────────────┐
│  代码段（只读物理页） │◄── 父进程 PTE（r-x）
│                     │◄── 子进程 PTE（r-x）  ← 直接共享，永不触发 COW
├─────────────────────┤
│  数据段物理页        │◄── 父进程 PTE（只读，COW 标记）
│ （原可写页）         │◄── 子进程 PTE（只读，COW 标记）  ← 共享，等待写触发
├─────────────────────┤
│  堆物理页            │◄── 父、子共享（COW 标记）
├─────────────────────┤
...
此时父子进程**共享同一份物理内存**，所有可写页被标记为只读。

---

#### 写入时发生什么：do_wp_page()

无论父进程还是子进程，任何一方写 COW 标记的页，都触发**写保护缺页中断（Protection Fault）**：

```
父进程写 data[0] = 99
       │
       ▼
CPU：PTE 标记只读，发出写操作 → 写保护缺页 #PF
       │
       ▼
内核 do_wp_page()
       │
       ├── page->_refcount > 1（有其他进程共享）
       │       │
       │       ▼
       │   ① 分配新物理页（alloc_zeroed_page）
       │   ② 将旧页内容复制到新页
       │   ③ 更新当前进程 PTE 指向新页，设置可写
       │   ④ 旧物理页引用计数 -1
       │
       └── page->_refcount == 1（已无人共享）
               │
               ▼
           直接将 PTE 改为可写（不需要复制）
...
if (pid == 0) {
    // 子进程
    // fork 后：g 所在页 PTE 标记只读，父子共享同一物理页
    g = 100;
    // → 写保护缺页 → do_wp_page() → 分配新页 → 子进程独立
    printf("子: g=%d\n", g);  // 100
} else {
    sleep(1);
    printf("父: g=%d\n", g);  // 仍是 42，物理页未受影响
...
| 维度 | 说明 |
|---|---|
| **fork 时间复杂度** | O(VMA 数量)，复制页表，不复制物理内存 |
| **谁先写，谁先复制** | 父子均可触发 COW，先写者先获得独立物理页 |
| **代码段** | 只读，父子永久共享，绝不触发 COW |
| **栈** | fork 后 COW 标记，子进程一旦执行（函数调用）即触发 |
| **exec() 后** | 所有 COW 共享页被完全替换为新程序映射，之前 COW 代价归零 |
| **vfork 区别** | 不复制页表，父子共享同一地址空间（含栈），子进程必须立即 exec/exit |
| **Redis BGSAVE** | fork 后父进程继续收写请求，每次写触发 COW 复制，高峰期可额外消耗数 GB 内存 |
| **THP 放大问题** | 开启透明大页（2MB）时，COW 每次复制 2MB 而非 4KB，Redis 官方建议关闭 THP |
