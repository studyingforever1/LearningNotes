...
**对于形状相同的岛屿，如果从同一起点出发，`dfs` 函数遍历的顺序肯定是一样的**。遍历顺序从某种意义上说就可以用来描述岛屿的形状

<img src=".\images\image-20260122091001912.png" alt="image-20260122091001912" style="zoom:33%;" />
...
这棵决策树，`第 i 层` 为 `第 i 个球` 做选择，可做的选择：`选择 1 or 2 or 3 号桶`，直到 `第 n 个球` 做完选择后结束

![image-20260126100019784](.\images\image-20260126100019784.png)
...
<img src=".\images\131.jpeg" style="zoom: 25%;" />
...
class Solution {
    ...
    private void backtrack(String s) {
        if (path.size() == 4 && s.isEmpty()) {
            String ip = path.stream().reduce("", (a, b) -> a + "." + b);
...
class Solution {
    ...
    public List<String> letterCombinations(String digits) {
        ...
        backtrack(0, digits, length);
        return res;
    }

    private void backtrack(int index, String digits, int length) {
        ...
        for (Character c : map.get(digits.charAt(index))) {
            path.append(c);
            backtrack(index + 1, digits, length);
            path.deleteCharAt(path.length() - 1);
        }
    }
}
...
<img src="D:\doc\my\studymd\LearningNotes\interview\images\662.png" style="zoom:50%;" />
...
<img src="D:\doc\my\studymd\LearningNotes\algorithm\images\310.jpg" style="zoom:67%;" />
...
class Solution {
    ...
    public int minFallingPathSum(int[][] matrix) {
        ...
        for (int i = 0; i < matrix[0].length; i++) {
            res = Math.min(dp(matrix.length - 1, i, matrix), res);
        }
        return res;
    }
...
class Solution {
    ...
    private boolean dp(String s, int i) {
        ...
        for (int len = 1; i + len <= s.length(); len++) {
            if (wordDictSet.contains(s.substring(i, i + len))) {
                boolean result = dp(s, i + len);
                if (result) {
                    memo[i] = 1;
                    return true;
                }
...
**以 `nums[i]` 为结尾的「最大子数组和」为 `dp[i]`**。`dp[i]` 有两种「选择」，要么与前面的相邻子数组连接，形成一个和更大的子数组；要么不与前面的子数组连接，自成一派，自己作为一个子数组。
...
class Solution {
    ...
    public int dp(String text1, int i, String text2, int j) {
        ...
        if (text1.charAt(i) == text2.charAt(j)) {
            ...
            memo[i][j] = min(
                    dp(text1, i, text2, j + 1) + 1,
                    dp(text1, i + 1, text2, j) + 1,
                    dp(text1, i + 1, text2, j + 1) + 2
            );
        }
        return memo[i][j];
    }
...
class Solution {
    ...
    public boolean canPartition(int[] nums) {
        ...
        return dp(nums.length - 1, sum / 2, nums);
    }

    private boolean dp(int i, int j, int[] nums) {
        if (i < 0) {
            return false;
        }
        if (j == 0) {
        ...
        memo[i][j] = dp(i - 1, j, nums) || dp(i - 1, j - nums[i], nums);
        return memo[i][j];
    }
}
```
