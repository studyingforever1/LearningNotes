...
SSD硬盘通常由重叠的一组盘片构成，每个盘面都被划分为数目相等的磁道，并从外缘的“0”开始编号，具有相同编号的磁道形成一个圆柱，称之为磁盘的柱面。

![](.\images\硬盘03.png)

> - **SSD**（固态硬盘）：基于闪存芯片存储数据，利用闪存芯片内的电信号来记录和读取数据。数据的写入和擦除通过控制闪存芯片内的晶体管来实现，具有快速的读写速度和随机访问能力。
> - **HDD**（机械硬盘）：通过磁头在高速旋转的盘片上进行数据的读写操作。盘片上涂有磁性材料，磁头根据电信号的变化在盘片上磁化不同的区域来表示数据，读取数据时则通过感应盘片上的磁场变化来获取信息。
>
> - 顺序读写速度
>   - **SSD**：顺序读取速度通常可达数百 MB/s 甚至数 GB/s，顺序写入速度也能达到几百 MB/s。例如，一些高端的 NVMe 协议的 M.2 接口 SSD，顺序读取速度可超过 3000MB/s，顺序写入速度可达 2000MB/s 以上。
>   - **HDD**：顺序读取速度一般在 100 - 200MB/s 左右，顺序写入速度略低于读取速度。
> - 随机读写速度
>   - **SSD**：4K 随机读取性能可以轻松达到几十 MB/s 甚至更高，4K 随机写入性能也能达到数 MB/s 到十几 MB/s 不等。这使得 SSD 在启动系统、加载应用程序以及处理大量小文件时表现出色。
>   - **HDD**：随机读写性能较差，4K 随机读取速度通常在 1MB/s 以下，随机写入速度也很低。这是因为 HDD 的机械结构决定了其在随机访问数据时需要频繁移动磁头，导致寻道时间较长。
