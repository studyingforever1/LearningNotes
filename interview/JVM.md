...
Young GC 后，Eden + Survivor 中存活的对象要复制到另一个 Survivor，但如果存活对象**太多，Survivor 放不下**怎么办？

这时候需要**老年代**来兜底，这就是空间分配担保。

**JDK 6u24 之后的变化**

`HandlePromotionFailure` 这个参数**被废弃**了，规则简化为：
...
冒险执行了 Minor GC
        ↓
Survivor 真的放不下，老年代也不够
        ↓
担保失败（Promotion Failed）
        ↓
立即触发 Full GC
        ↓
Full GC 后还不够
...
> 如果触发Full GC会使用单线程的标记整理整个堆。JDK10以后是多线程的。
>
...
<img src="D:\doc\my\studymd\LearningNotes\interview\images\object_lifecycle_g1.svg" style="zoom: 25%;" />



**G1 平时做三件事，按频率从高到低：**

<img src=".\images\image-20260513173552490.png" alt="image-20260513173552490" style="zoom:50%;" />

**① Young GC（每隔几秒就来一次）**

新对象不断涌入 Eden，Eden 满了就触发。整个过程 STW（全城暂停），把 Eden + Survivor 里还活着的对象复制到新的 Survivor 或 Old，然后把 Eden 地块全部清空变成 Free。

这是最普通的一次垃圾回收，跟其他收集器的 Minor GC 一个意思。

**② 并发标记（每隔一段时间，Old 区压力大时）**

Old 区对象越积越多，G1 需要搞清楚"老城区哪些地块垃圾最多、值得拆迁"。于是派出侦察队（后台线程），在不停业的情况下把整个堆扫一遍，给每个 Old Region 标上"垃圾密度"。

这个过程大部分时间与用户线程并发，只有开始和结束各停一小下。**它本身不回收任何东西，只是调查。**

**③ Mixed GC（并发标记完后，接着来几轮）**

侦察结果出来了，G1 知道哪些 Old Region 垃圾最多。于是执行拆迁：

- 选定 CSet = 全部 Young Region + 垃圾最多的几个 Old Region
- STW，把 CSet 里还活着的对象复制到 Free Region
- 整个 CSet 清空，变成 Free

这就是"混合"的含义——同时拆年轻区和部分老城区。每次拆几个 Old Region 由停顿时间预算控制，分多轮完成。

<img src=".\images\image-20260513173425675.png" alt="image-20260513173425675" style="zoom: 50%;" />
...
对于初始化阶段，虚拟机严格规范了有且仅有 6 种情况下，必须对类进⾏初始化

1. 遇到 new 、 getstatic 、 putstatic 或 invokestatic 这 4 条字节码指令时：

   new : 创建⼀个类的实例对象。 

   getstatic 、 putstatic : 读取或设置⼀个类型的静态字段（被 final 修饰、已在编译期把结 果放⼊常量池的静态字段除外）。 

   invokestatic : 调⽤类的静态⽅法。

2. 使⽤ java.lang.reflect 包的⽅法对类进⾏反射调⽤时如 Class.forName("...") , newInstance() 等 等。如果类没初始化，需要触发其初始化。

3. 初始化⼀个类，如果其⽗类还未初始化，则先触发该⽗类的初始化。

4. 当虚拟机启动时，⽤户需要定义⼀个要执⾏的主类 (包含 main ⽅法的那个类)，虚拟机会先初 始化这个类。

5. MethodHandle 和 VarHandle 可以看作是轻量级的反射调⽤机制，⽽要想使⽤这 2 个调⽤，就 必须先使⽤ findStaticVarHandle 来初始化要调⽤的类。

6. 当⼀个接⼝中定义了 JDK8 新加⼊的默认⽅法（被 default 关键字修 饰的接⼝⽅法）时，如果有这个接⼝的实现类发⽣了初始化，那该接⼝要在其之前被初始化。
