...
- `写-写`情况：即并发事务相继对相同的记录做出改动。
  ...
  <img src="./images/image-20251022111507800.png" alt="image-20251022111507800" style="zoom:33%;" />
...
- `读-写`或`写-读`情况：也就是一个事务进行读取操作，另一个进行改动操作。
  ...
  - 方案一：读操作利用多版本并发控制（`MVCC`），写操作进行`加锁`。
    ...
    查询语句只能读到在生成`ReadView`之前已提交事务所做的更改，在生成`ReadView`之前未提交的事务或者之后才开启的事务所做的更改是看不到的。而写操作肯定针对的是最新版本的记录，读记录的历史版本和改动记录的最新版本本身并不冲突，也就是采用`MVCC`时，`读-写`操作并不冲突。

  - 方案二：读、写操作都采用`加锁`的方式。

    在读取记录的时候也就需要对其进行`加锁`操作，这样也就意味着`读`操作和`写`操作也像`写-写`操作那样排队执行。
...
并发事务的`读-读`情况并不会引起什么问题，不过对于`写-写`、`读-写`或`写-读`这些情况可能会引起一些问题，需要使用`MVCC`或者`加锁`的方式来解决它们。

在使用`加锁`的方式解决问题时，由于既要允许`读-读`情况不受影响，又要使`写-写`、`读-写`或`写-读`情况中的操作相互阻塞，锁分类如下：
...
前面提到的`锁`都是针对记录的，也可以被称之为`行级锁`或者`行锁`，对一条记录加锁影响的也只是这条记录而已，我们就说这个锁的粒度比较细；其实一个事务也可以在`表`级别进行加锁，自然就被称之为`表级锁`或者`表锁`，对一个表加锁影响整个表中的记录，我们就说这个锁的粒度比较粗。
...
`InnoDB`存储引擎既支持表锁，也支持行锁。表锁实现简单，占用资源较少，不过粒度很粗。行锁粒度更细，可以实现更精准的并发控制
...
- 表级别的`S锁`、`X锁`
  ...
  - `LOCK TABLES t WRITE`：`InnoDB`存储引擎会对表`t`加表级别的`X锁`。
    ...
      不过请尽量避免在使用`InnoDB`存储引擎的表上使用`LOCK TABLES`这样的手动锁表语句，它们并不会提供什么额外的保护，只是会降低并发能力而已。`InnoDB`的厉害之处还是实现了更细粒度的行锁，关于表级别的`S锁`和`X锁`了解一下就罢。
...
- `Record Locks`：`记录锁`，仅仅把一条记录锁上，官方的类型名称为：`LOCK_REC_NOT_GAP`。

  <img src="./images/image-20251022161819384.png" alt="image-20251022161819384" style="zoom:33%;" />

  `记录锁`是有`S锁`和`X锁`之分的，当一个事务获取了一条记录的`S型记录锁`后，其他事务也可以继续获取该记录的`S型记录锁`，但不可以继续获取`X型记录锁`；当一个事务获取了一条记录的`X型记录锁`后，其他事务既不可以继续获取该记录的`S型记录锁`，也不可以继续获取`X型记录锁`；

- `Gap Locks`：官方的类型名称为：`LOCK_GAP`，简称为`gap锁`。

  `MySQL`在`REPEATABLE READ`隔离级别下是可以解决幻读问题的，解决方案有两种，可以使用`MVCC`方案解决，也可以采用`加锁`方案解决。但是在使用`加锁`方案解决时，事务在第一次执行读取操作时，那些幻影记录尚不存在，我们无法给这些幻影记录加上`记录锁`。

  <img src="./images/image-20251022162253593.png" alt="image-20251022162253593" style="zoom:33%;" />
  ...
  >
  >   通过给这两条记录加gap锁可以实现给（-∞，1）和（20，+∞）间隙加锁

  **`gap锁`的提出仅仅是为了防止插入幻影记录，不会限制修改现有记录，也不会与其他 GAP 锁冲突。**

- `Next-Key Locks`：官方的类型名称为：`LOCK_ORDINARY`，我们也可以简称为`next-key锁`

  有时候我们既想锁住某条记录，又想阻止其他事务在该记录前面的`间隙`插入新记录

  <img src="./images/image-20251022163238502.png" alt="image-20251022163238502" style="zoom:33%;" />

  `next-key锁`的本质就是一个`记录锁`和一个`gap锁`的合体，它既能保护该条记录，又能阻止别的事务将新记录插入被保护记录前面的`间隙`。

- `Insert Intention Locks`：官方的类型名称为：`LOCK_INSERT_INTENTION`，我们也可以称为`插入意向锁`。

  ​	一个事务在插入一条记录时需要判断一下插入位置是不是被别的事务加了所谓的`gap锁`，如果有的话，插入操作需要等待，直到拥有`gap锁`的那个事务提交。但是`InnoDB`规定事务在等待的时候也需要在内存中生成一个`锁结构`，表明有事务想在某个`间隙`中插入新记录，但是现在在等待。这种类型的锁命名为`Insert Intention Locks`

  <img src="./images/image-20251022163720973.png" alt="image-20251022163720973" style="zoom:33%;" />
  ...
  <img src="./images/image-20251022163750023.png" alt="image-20251022163750023" style="zoom:33%;" />

- 隐式锁

   我们前面说一个事务在执行`INSERT`操作时，如果即将插入的`间隙`已经被其他事务加了`gap锁`，那么本次`INSERT`操作会阻塞，并且当前事务会在该间隙上加一个`插入意向锁`，否则一般情况下`INSERT`操作是不加锁的。
   ...
   - 对于二级索引记录来说，本身并没有`trx_id`隐藏列，但是在二级索引页面的`Page Header`部分有一个`PAGE_MAX_TRX_ID`属性，该属性代表对该页面做改动的最大的`事务id`，如果`PAGE_MAX_TRX_ID`属性值小于当前最小的活跃`事务id`，那么说明对该页面做修改的事务都已经提交了，否则就需要在页面中定位到对应的二级索引记录，然后回表找到它对应的聚簇索引记录，然后再重复`情景一`的做法。
...
在对不同记录加锁时，如果符合下面这些条件，那么这些记录的锁就可以被放到一个`锁结构`中
...
<img src="./images/image-20251023094847548.png" alt="image-20251023094847548" style="zoom:33%;" />
...
<img src="./images/woik2rylybks2_5f5d260d9a3348eba6d785cb2d19ecb4.png" alt="img" style="zoom:50%;" />
...
- `一堆比特位`：
  ...
  页面中的每条记录在`记录头信息`中都包含一个`heap_no`属性，伪记录`Infimum`的`heap_no`值为`0`，`Supremum`的`heap_no`值为`1`，之后每插入一条记录，`heap_no`值就增1。`锁结构`最后的一堆比特位就对应着一个页面中的记录，一个比特位映射一个`heap_no`

<img src="./images/image-20251023101623747.png" alt="image-20251023101623747" style="zoom:33%;" />
...
- 在Read Uncommited隔离级别下，不加锁，直接读取记录的最新版本；可能出现脏读、不可重复读、幻读现象。

- 在Read Commited隔离级别下，不加锁；在每次执行普通的select语句时都会生成一个ReadView，这样避免了脏读现象，但没有避免不可重复读和幻读现象。

- 在Repeatable Read隔离级别下，不加锁；只在第一次执行普通select语句时生成一个ReadView，这样就把脏读、不可重复读和**大部分**幻读现象都避免了。

  在Repeatable Read隔离级别下，还是可能出现幻读现象，例如：
  ...
  #此时事务T2提交了 insert into hero values (30, 'g关羽', '蜀');
  
  update hero set name = '关羽' where number = 30;
  select  * from hero where number = 30;
  commit;
  ```

  虽然T1事务全程只生成了一个ReadView，但是ReadView不能阻止T1修改number=30的记录，当number=30的记录被T1修改后，记录的隐藏列中`trx_id`的值就被赋值为T1的事务id，此时再次进行select查询，当前记录的事务id是本事务，对本事务可见，出现幻读。

- 在Serializable隔离级别下，需要分两种情况：

  - 当系统变量autocommit=0时（禁用自动提交），普通的select语句会被转换成`select ... lock in share mode`。需要先获取当前记录的S锁，加锁情况与在Repeatable Read隔离级别下一样。
  - 当系统变量autocommit=1时（启用自动提交），普通的select语句不会加锁，只会利用MVCC生成一个ReadView读取记录。
...
语句1和语句2是MySQL中规定的两种锁定读的语法格式，而语句3和语句4由于在执行的过程中需要首先定位到被改动的记录并给记录加锁，因此也是一种锁定读。

###### 匹配模式

在使用索引执行查询时，查询优化器会生成若干个扫描区间，针对每一个扫描区间，都可以快速定位到第一条记录，然后沿着这条记录的单链表扫描区间内的其他记录，直到某条记录不在区间范围内为止。

如果扫描的区间是一个单点扫描区间，此时的匹配模式为精确匹配

- 如果边界条件为a=1，那么扫描区间就是[1,1]，Innodb认为这是一个单点区间，此时的匹配模式就是精确匹配
- 如果边界条件为a=1 AND b=1，那么扫描区间就是[(1,1),(1,1)]，Innodb认为这是一个单点区间，此时的匹配模式也是精确匹配
- 如果边界条件为a=1 AND b>=1，那么扫描区间就是[(1,1),(1,+∞)]，此时的匹配模式就不是精确匹配。

###### 唯一性搜索

在扫描某个区间的记录前，能够确定该扫描区间的记录最多只有1条，那么就把这种情况称作唯一性搜索。满足以下条件的为唯一性搜索：

- 匹配模式为精确匹配
- 使用的索引是唯一二级索引，搜索条件不能为“索引列 is null” （唯一二级索引允许多个null值）
- 如果索引中包含多个列，那么每一个列都应该被用到



###### 锁定读分析

“XX语句给哪些记录加什么锁”，依赖于多种条件：

- 事务的隔离级别
- 语句执行时使用的索引类型（聚簇索引、唯一二级索引、普通二级索引）
- 是否是精确匹配
- 是否是唯一性搜索
- 具体执行的语句类型（select、insert、update、delete）

事务在执行过程中所获取的锁一般在事务提交/回滚时才会释放，但是在隔离级别不大于Read Commited时，为了提高低隔离级别下的并发能力，某些情况也会提前将一些不符合搜索条件的锁释放掉。

锁定读的执行可以看成是依次读取若干个扫描区间的记录，读取某个扫描区间中记录的过程如下：

1. 快速的在B+树叶子节点定位到该扫描区间的第一条记录，把该记录作为当前记录。

2. 为当前记录加锁

   一般情况下，对于锁定读的语句，在隔离级别不大于Read Commited时，会为当前记录加记录锁。

   在隔离级别不小于Repeatable Read时，会为当前记录加next-key锁

3. 判断索引条件下推的条件是否成立

   如果当前记录符合索引下推的条件，则跳到步骤4继续；

   如果不符合，则直接获取到当前记录所在单链表的下一条记录，将该记录作为新的当前记录，跳到步骤2；

   当前步骤还会判断当前记录是否符合形成扫描区间的边界条件，如果不符合则跳过步骤4和5，直接给server层返回”查询完毕“。

   当前步骤不会释放锁

4. 执行回表操作

   如果读取的是二级索引记录，则需要进行回表操作，获取到对应的聚簇索引并给该聚簇索引记录加记录锁

5. 判断边界条件是否成立

   如果该记录符合边界条件，则跳到步骤6继续执行，否则在隔离级别不大于Read Commited时，要释放掉加在该记录上的锁，在隔离级别不小于Repeatable Read时，不释放锁，并向server层返回一个”查询完毕“的信息。

6. server层判断其他条件是否成立

   除了索引下推的条件以外，server层还要判断其他搜索条件是否成立。如果成立，则将该记录发送到客户端，否则在隔离级别不大于Read Commited时，要释放掉加在该记录上的锁，在隔离级别不小于Repeatable Read时，不释放锁

7. 获取当前记录所在单链表的下一条记录，并将其作为新的当前记录，并跳回步骤2



对于`update`和`delete`语句，加锁方式与`select ... lock in share mode`和`select ... for update` 语句类似，不同的是如果更新了二级索引列，所有被更新的二级索引列在更新前都要加x型记录锁。



**特殊情况**

- 当隔离级别不大于Read Commited时，如果匹配模式为精确匹配，则不会为扫描区间后面的下一条记录加锁。
- 当隔离级别不小于Repeatable Read时，如果匹配模式为精确匹配，则会为扫描区间后面的下一条记录加gap锁。（有时扫描区间没有记录，也要加一个gap锁）
- 当隔离级别不小于Repeatable Read时，如果匹配模式不是精确匹配，并且没有找到匹配的记录，则会为该扫描区间的下一条记录加next-key锁
- 当隔离级别不小于Repeatable Read时，如果使用的是聚簇索引，并且扫描区间是左闭区间，而且定位到的第一条聚簇索引的number值正好与扫描区间中的最小值相同，那么会为该聚簇索引记录加记录锁。
- 无论是哪个隔离级别，只要是唯一性搜索，并且读取的记录没有被标记为已删除，就为读取到的记录加记录锁。
- 在扫描某个区间的记录时，一般都是按照从左往右的顺序进行扫描，如果某些情况下需要从右往左扫描，当隔离级别不小于Repeatable Read时，会给匹配到的第一条记录的下一条记录加gap锁。



##### 半一致性读

半一致性读是一种夹在一致性读和锁定读之间的读取方式，当隔离级别不大于Read Commited且执行`update`语句时将使用半一致性读。

当`update`语句读取到已经被其他事务加了X锁的记录时，Innodb会将该记录的最新提交版本读出来，然后判断该版本是否与`update`语句中的搜索条件相匹配。如果不匹配，不对该记录加锁，跳到下一条记录。如果匹配，则再次读取该记录并且对其进行加锁等待。这样可以让`update`语句尽量减少被其他语句阻塞。

##### insert语句

除了隐式锁和意向插入锁，执行insert语句时，会在内存中生成锁结构的两种特殊情况：

- 遇到重复键

  在插入一条新纪录时，如果发现现有记录的主键与待插入记录的主键列相同，在报错前，对聚簇索引加锁

  - 当隔离级别不大于Read Commited时，加的是S型记录锁。
  - 当隔离级别不小于Repeatable Read时，加的是S型next-key锁。

  在插入一条新纪录时，如果发现现有记录的唯一二级索引列与待插入记录的唯一二级索引列相同，在报错前，对唯一二级索引加锁

  ​	无论是哪个隔离级别，都会加一个next-key锁的S锁。

  ​	如果是`INSERT ... ON DUPLICATE KEY UPDATE`语法，那么会对B+树的记录加X锁，而不是S锁。

- 外键检查

  - 待插入外键表的记录的外键值能在主表中找到

    在插入成功之前，无论当前事务的隔离级别是什么，给主表的记录加上一个S型的记录锁

  - 待插入外键表的记录的外键值能在主表中找不到

    在插入成功之前，需要根据隔离级别对主表的记录加锁

    - 当隔离级别不大于Read Commited时，并不对记录加锁
    - 当隔离级别不小于Repeatable Read时，加的是gap锁
...
```mysql
show engine innodb status;#能看到最近一次死锁发生的信息
#将innodb_print_all_deadlocks设置为ON 可以将每次死锁发生的信息打印到错误日志中
```
...
------------------------
2025-10-24 14:31:11 139890037618240
*** (1) TRANSACTION:
TRANSACTION 1598, ACTIVE 24 sec starting index read
mysql tables in use 1, locked 1
LOCK WAIT 3 lock struct(s), heap size 1128, 2 row lock(s)
MySQL thread id 54, OS thread handle 139889977816640, query id 3363 172.16.16.35 root statistics
/* ApplicationName=DataGrip 2025.2.4 */ select * from hero where number = 3 for update

*** (1) HOLDS THE LOCK(S):
RECORD LOCKS space id 6 page no 4 n bits 72 index PRIMARY of table `test`.`hero` trx id 1598 lock_mode X locks rec but not gap
Record lock, heap no 2 PHYSICAL RECORD: n_fields 5; compact format; info bits 0
 0: len 4; hex 80000001; asc     ;;
 1: len 6; hex 00000000062e; asc      .;;
 2: len 7; hex 82000000da0110; asc        ;;
 3: len 7; hex 6ce58898e5a487; asc l      ;;
 4: len 3; hex e89c80; asc    ;;


*** (1) WAITING FOR THIS LOCK TO BE GRANTED:
RECORD LOCKS space id 6 page no 4 n bits 72 index PRIMARY of table `test`.`hero` trx id 1598 lock_mode X locks rec but not gap waiting
Record lock, heap no 3 PHYSICAL RECORD: n_fields 5; compact format; info bits 0
 0: len 4; hex 80000003; asc     ;;
 1: len 6; hex 00000000062e; asc      .;;
 2: len 7; hex 82000000da011d; asc        ;;
 3: len 10; hex 7ae8afb8e8919be4baae; asc z         ;;
 4: len 3; hex e89c80; asc    ;;


*** (2) TRANSACTION:
TRANSACTION 1599, ACTIVE 15 sec starting index read
mysql tables in use 1, locked 1
LOCK WAIT 3 lock struct(s), heap size 1128, 2 row lock(s)
MySQL thread id 62, OS thread handle 139889975707200, query id 3374 172.16.16.35 root statistics
/* ApplicationName=DataGrip 2025.2.4 */ select * from hero where number = 1 for update

*** (2) HOLDS THE LOCK(S):
RECORD LOCKS space id 6 page no 4 n bits 72 index PRIMARY of table `test`.`hero` trx id 1599 lock_mode X locks rec but not gap
Record lock, heap no 3 PHYSICAL RECORD: n_fields 5; compact format; info bits 0
 0: len 4; hex 80000003; asc     ;;
 1: len 6; hex 00000000062e; asc      .;;
 2: len 7; hex 82000000da011d; asc        ;;
...
RECORD LOCKS space id 6 page no 4 n bits 72 index PRIMARY of table `test`.`hero` trx id 1599 lock_mode X locks rec but not gap waiting
Record lock, heap no 2 PHYSICAL RECORD: n_fields 5; compact format; info bits 0
 0: len 4; hex 80000001; asc     ;;
 1: len 6; hex 00000000062e; asc      .;;
 2: len 7; hex 82000000da0110; asc        ;;
